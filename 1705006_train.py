...
def get_data(paths_img,path_label=None,RESIZE_IMAGE=None):
    ...
    for i,path in enumerate(paths_img):
        ...
        if RESIZE_IMAGE is not None:
            img=cv2.resize(img,(RESIZE_IMAGE,RESIZE_IMAGE),interpolation=cv2.INTER_AREA) # resize image to 28x28
...
class CNN:
    ...
    def predict(self,X):
        return self.forward_pass(X).transpose()
...
def main():
    ...
    X_train_a,y_train_a=get_data(paths_train_a,path_label_train_a,RESIZE_IMAGE=RESIZE_IMAGE)
    X_train_b,y_train_b=get_data(paths_train_b,path_label_train_b,RESIZE_IMAGE=RESIZE_IMAGE)
    X_train_c,y_train_c=get_data(paths_train_c,path_label_train_c,RESIZE_IMAGE=RESIZE_IMAGE)

    indices=list(range(len(X_train_a)))
    ind=int(len(indices)*0.1)

    X_train_a=X_train_a[indices[:ind]] 
    y_train_a=y_train_a[indices[:ind]]

    indices=list(range(len(X_train_b)))
    ind=int(len(indices)*0.4)

    X_train_b=X_train_b[indices[:ind]]
    y_train_b=y_train_b[indices[:ind]]

    indices=list(range(len(X_train_c)))
    ind=int(len(indices)*0.01)
    X_train_c=X_train_c[indices[:ind]]
    y_train_c=y_train_c[indices[:ind]]


    # X_train_all=X_train_b
    # y_train_all=y_train_b
    ...
    model.add(Relu_Layer())  
    model.add(Fully_Connected_Layer(num_classes,256))
